...
#' @importFrom dplyr tibble summarise across all_of
...
validate_student_responses <- function(df) {
  expected_schema <- dplyr::tibble(
    ...
    item_id = numeric(),
    response = list(),
    # Can be character, multiple responses, or long text
  ...
  if (any(type_mismatch)) {
    rlang::abort(
      glue::glue(
        "Error: The following columns have incorrect types: {paste(names(df)[type_mismatch], collapse = ', ')}"
      )
    )
  }
  ...
  if (any(missing_values > 0)) {
    warning(
      glue::glue(
        "Warning: Missing values detected in critical columns: {paste(names(missing_values)[missing_values > 0], collapse = ', ')}"
      )
    )
  }
